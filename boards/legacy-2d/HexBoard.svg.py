...
def calc_edges(n):
    ...
    edges = np.vstack([edges + ii*(1+2j) for ii in range(n)])
    edges = np.vstack([edges + ii*(1-1j) for ii in range(n+1)])
    return edges
...
def addCenters(d,n):
    centers = np.array([1+1j + ii*(1+2j) for ii in range(n)])
    centers = np.hstack([centers + ii*(1-1j) for ii in range(n+1)])
...
if __name__ == '__main__':
    ...
    addCenters(d, n)
    d.save_svg(str(_OUT / 'HexBoard.svg'))
